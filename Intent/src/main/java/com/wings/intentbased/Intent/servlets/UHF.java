...
import blueprints.UHFBp;

/**
 * Servlet implementation class UHF
 */
public class UHF extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		UHFBp uhf = new UHFBp();
		uhf.setName("Ultra High-Fidelity Media");
		uhf.setDescription("Live and on-demand channels for high quality TV (up to 4K video service)");
		uhf.setVersion(1.0);
		uhf.setIdentity("BP Identity");
		uhf.setSector("Media & Entertainment");
		uhf.setService("eMBB");
		uhf.setCountry("Spain");
		uhf.setLatency(100);
		uhf.setDataRate(120);
		uhf.setReliability(99.99);
		uhf.setAvailability(99.99);
		uhf.setMobility(50);
		uhf.setBroadband(249);
		uhf.setCapacity(1.529);
		uhf.setDensity(13804);
		uhf.setSlicing("Y");
		uhf.setSecurity("N");
		uhf.setDuration(80);
		...
		//update of values according to edit values
			...
			uhf.setFromCountry(countryFrom);

			//uhf.setCellBand(Integer.parseInt(cellBandCh));
			uhf.setDay(Integer.parseInt(dayCh));
			uhf.setMonth(Integer.parseInt(monthCh));
			uhf.setYear(Integer.parseInt(yearCh));
			uhf.setHour(Integer.parseInt(hourCh));
			uhf.setMinutes(Integer.parseInt(minutesCh));
			uhf.setLatency(Integer.parseInt(latencyCh));
			uhf.setDataRate(Integer.parseInt(dataRateCh));
			uhf.setReliability(Double.parseDouble(reliabilityCh));
			uhf.setAvailability(Double.parseDouble(availabilityCh));
			uhf.setBroadband(Double.parseDouble(broadbandCh));
			uhf.setMobility(Integer.parseInt(mobilityCh));
			uhf.setCapacity(Double.parseDouble(capacityCh));
			uhf.setDensity(Integer.parseInt(densityCh));
			uhf.setSlicing(slicingCh);
			uhf.setSecurity(securityCh);
			
			if(uhf.getMinutes()<10)
				request.setAttribute("minutes", "0" + uhf.getMinutes());
			else
				request.setAttribute("minutes", uhf.getMinutes());
			if(uhf.getHour()<10)
				request.setAttribute("hour", "0" + uhf.getHour());
			else
				request.setAttribute("hour", uhf.getHour());
			
			request.setAttribute("uhf", uhf);
			request.getRequestDispatcher("UHFEdit.jsp").forward(request, response);
		}
		...
		//insert of experiment in database if slot available else redirection to edit page
			...
			if(cd.dateCompare(Integer.parseInt(dayCh), Integer.parseInt(monthCh), Integer.parseInt(yearCh)) == 0) {
				uhf.setDay(Calendar.getInstance().get(Calendar.DAY_OF_MONTH));
				uhf.setMonth(Calendar.getInstance().get(Calendar.MONTH) + 1);
				uhf.setYear(Calendar.getInstance().get(Calendar.YEAR));
				if(Calendar.getInstance().get(Calendar.HOUR_OF_DAY)<23) {
					uhf.setHour(Calendar.getInstance().get(Calendar.HOUR_OF_DAY)+1);
					uhf.setMinutes(00);
				}
				else
				{
				...
				if(ct.timeCompare(uhf.getHour(), uhf.getMinutes()) == 0) {
					if(Calendar.getInstance().get(Calendar.HOUR_OF_DAY)<23) {
						uhf.setHour(Calendar.getInstance().get(Calendar.HOUR_OF_DAY)+1);
						uhf.setMinutes(00);
					}
			...
			request.setAttribute("uhf", uhf);
			request.getRequestDispatcher("UHFConfirmed.jsp").forward(request, response);
			
			/*Database db = new Database();
			
			String query;
			if(uhf.getMinutes() < 10)
				query = "SELECT name, dateOfExecution, timeOfExecution, launchedTo FROM events WHERE dateOfExecution='" + uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear() +"' AND launchedTo='" + uhf.getCountry() + "' AND endTimeOfExecution < '" + Double.parseDouble(uhf.getHour() + ".0" + uhf.getMinutes()) + "' AND timeOfExecution >= '" + db.estimateEndTime(Double.parseDouble(uhf.getHour() + ".0" + uhf.getMinutes()), uhf.getDuration()) + "' ORDER BY timeOfExecution;";
			else
				query = "SELECT name, dateOfExecution, timeOfExecution, launchedTo FROM events WHERE dateOfExecution='" + uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear() +"' AND launchedTo='" + uhf.getCountry() + "' AND endTimeOfExecution < '" + Double.parseDouble(uhf.getHour() + "." + uhf.getMinutes()) + "' AND timeOfExecution >= '" + db.estimateEndTime(Double.parseDouble(uhf.getHour() + "." + uhf.getMinutes()), uhf.getDuration()) + "' ORDER BY timeOfExecution;";
			
			ResultSet rs = db.getResult(query);
			
			double dtime;
			if(uhf.getMinutes() < 10)
				dtime = Double.parseDouble(uhf.getHour() + ".0" + uhf.getMinutes());
			else
				dtime = Double.parseDouble(uhf.getHour() + "." + uhf.getMinutes());
			
			try {
				if(!rs.next() && db.checkAvailability(uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear(), dtime, uhf.getDuration(), uhf.getCountry(), db) == true)
				{
					double endTime = db.estimateEndTime(dtime, uhf.getDuration());
					rs.beforeFirst();
					System.out.println("End time " + endTime);
					if(uhf.getMinutes() < 10)
						db.insertIntoDB(uhf.getName(), uhf.getDescription(), uhf.getVersion(), uhf.getIdentity(), uhf.getSector(), uhf.getFromCountry(), uhf.getCountry(), uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear(), Double.parseDouble(uhf.getHour() + ".0" + uhf.getMinutes()), endTime, uhf.getDuration(), 0, uhf.getService(), uhf.getLatency(), uhf.getDataRate(), uhf.getReliability(), uhf.getAvailability(), uhf.getBroadband(), uhf.getMobility(), uhf.getCapacity(), uhf.getDensity(), uhf.getSlicing(), uhf.getSecurity());
					else
						db.insertIntoDB(uhf.getName(), uhf.getDescription(), uhf.getVersion(), uhf.getIdentity(), uhf.getSector(), uhf.getFromCountry(), uhf.getCountry(), uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear(), Double.parseDouble(uhf.getHour() + "." + uhf.getMinutes()), endTime, uhf.getDuration(), 0, uhf.getService(), uhf.getLatency(), uhf.getDataRate(), uhf.getReliability(), uhf.getAvailability(), uhf.getBroadband(), uhf.getMobility(), uhf.getCapacity(), uhf.getDensity(), uhf.getSlicing(), uhf.getSecurity());
					request.setAttribute("uhf", uhf);
					request.getRequestDispatcher("UHFConfirmed.jsp").forward(request, response);
				}
				else
				{
					...
					String av1 = db.nextAvailable(uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear(), Double.parseDouble(uhf.getHour() + "." + uhf.getMinutes()), uhf.getDuration(), "Spain", db);
					System.out.println("av1: " +av1);
					
					if(uhf.getMinutes() < 10)
						request.setAttribute("notAvailable", "Service not available in " + uhf.getCountry() + " at " + uhf.getHour() + ":0" + uhf.getMinutes() + " on " + uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear() + "!");
					else
						request.setAttribute("notAvailable", "Service not available in " + uhf.getCountry() + " at " + uhf.getHour() + ":" + uhf.getMinutes() + " on " + uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear() + "!");
					...
					{
						request.setAttribute("suggestion", "You can try in " + av1 + " on " + uhf.getDay() + "/" + uhf.getMonth() + "/" + uhf.getYear() + ".");
					}
					request.setAttribute("uhf", uhf);
					request.getRequestDispatcher("UHFEdit.jsp").forward(request, response);
				}
		...
		//getting values from guided selection
			...
			request.setAttribute("uhf", uhf);
			request.getRequestDispatcher("UHFCreated.jsp").forward(request, response);
		}
		...
		//checks if command contains information about the country to run the experiment from
			...
			if(fcount1 == null) {
				...
				request.getRequestDispatcher("UHFFromCountry.jsp").forward(request, response);
			}
			else if(fcount2 == null) {
				uhf.setFromCountry(fcount1);
				fcountry = true;
			}
			else if(fcount2 != null) {
				...
				request.setAttribute("intent", intent);
				request.setAttribute("servlet", "UHF");
				request.getRequestDispatcher("UHFFromCountry.jsp").forward(request, response);
			...
			//checks if command contains information about the bandwidth
				/*if(fcountry == true) {
					...
					if(num > 1) {
						...
						request.setAttribute("intent", intent);
						request.getRequestDispatcher("UHFBandwidth.jsp").forward(request, response);
					}
					...
					if(intent.toLowerCase().indexOf("$")!=-1) {
						...
						if(tmp.equalsIgnoreCase("") == false) {
							if(Double.parseDouble(tmp)>99 && Integer.parseInt(tmp)<201) {
								uhf.setCellBand(Integer.parseInt(tmp));
								cellband = true;
							}
							else {
			...
			//checks if command contains information about the date to run the experiment
				if(fcountry == true) {
					...
					if(num == 1 || num > 2) {
						...
						request.setAttribute("intent", intent);
						request.getRequestDispatcher("UHFDate.jsp").forward(request, response);
					}
					...
					if(num == 2 && intent.toLowerCase().indexOf("/")!=-1) {
						...
						if(day.equalsIgnoreCase("") == false) {
							if(Integer.parseInt(day) > 0 && Integer.parseInt(day) < 32) {
								uhf.setDay(Integer.parseInt(day));
							}
							else uhf.setDay(Calendar.getInstance().get(Calendar.DAY_OF_MONTH));
						...
						if(month.equalsIgnoreCase("") == false) {
							if(Integer.parseInt(month)>0 && Integer.parseInt(month)<13) {
								uhf.setMonth(Integer.parseInt(month));
							}
							else
								uhf.setMonth(Calendar.getInstance().get(Calendar.MONTH) + 1);
						}
						...
						if(year.equalsIgnoreCase("") == false) {
							if(Integer.parseInt(year)>=Calendar.getInstance().get(Calendar.YEAR)) {
								uhf.setYear(Integer.parseInt(year));
							}
							else
								uhf.setYear(Calendar.getInstance().get(Calendar.YEAR));
						}
						...
						String givda = ""+uhf.getYear()+"-"+uhf.getMonth()+"-"+uhf.getDay()+"";
						try {
							...
							day = Integer.toString(uhf.getDay());
							month = Integer.toString(uhf.getMonth());
							year = Integer.toString(uhf.getYear());
							...
							else if(cd.dateCompare(uhf.getDay(), uhf.getMonth(), uhf.getYear()) == 0) {
				...
				if(date == true) {
					...
					if(num > 1) {
						...
						request.setAttribute("intent", intent);
						request.getRequestDispatcher("UHFTime.jsp").forward(request, response);
					}
					...
					if(intent.toLowerCase().indexOf(":")!=-1) {
						...
						if(hour.equalsIgnoreCase("") == false) {
							if(Integer.parseInt(hour) > -1 && Integer.parseInt(hour) < 24)
								uhf.setHour(Integer.parseInt(hour));
							else
								uhf.setHour(Calendar.getInstance().get(Calendar.HOUR_OF_DAY));
						}
						else
							uhf.setHour(Calendar.getInstance().get(Calendar.HOUR_OF_DAY));
						...
						if(minutes.equalsIgnoreCase("") == false) {
							if(Integer.parseInt(minutes) > -1 && Integer.parseInt(minutes) < 60) 
								uhf.setMinutes(Integer.parseInt(minutes));
							else
								uhf.setMinutes(00);	
						}
						
						if(cd.dateCompare(uhf.getDay(), uhf.getMonth(), uhf.getYear()) == 1) {
							if(ct.timeCompare(uhf.getHour(), uhf.getMinutes()) == 0) {
								if(Calendar.getInstance().get(Calendar.HOUR_OF_DAY) == 23) {
									uhf.setHour(00);
									uhf.setMinutes(00);
									uhf.setDay(uhf.getDay()+1);
									time = true;
								}
								else {
									uhf.setHour(Calendar.getInstance().get(Calendar.HOUR_OF_DAY) + 1);
						...
						else if(cd.dateCompare(uhf.getDay(), uhf.getMonth(), uhf.getYear()) == 2) {
							time = true;
						}
						else
						{
				...
				if(fcountry == true && date == true && time == true) { //cellband == true && 
					if(uhf.getMinutes()<10)
						request.setAttribute("minutes", "0" + uhf.getMinutes());
					else
						request.setAttribute("minutes", uhf.getMinutes());
